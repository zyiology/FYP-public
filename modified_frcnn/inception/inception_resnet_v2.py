...
from inception.units import Conv2d, Reduction_A
...
class Inception_ResNetv2_multitask(nn.Module):
    def __init__(self, in_channels=3, classes=None, k=256, l=256, m=384, n=384):
        super(Inception_ResNetv2_multitask, self).__init__()

        assert isinstance(classes,dict), "classes should be a dict, with format {attribute_name: num_classes}"
        ...
        #     attrib_name: nn.Linear(1536, num_classes) for attrib_name, num_classes in classes.items()
        # })

        self.linears = nn.ModuleDict({
            attrib_name: AttribLinear(1536, 512, num_classes) for attrib_name, num_classes in classes.items()
        })
    ...
    def forward(self, x):
        ...
        logits = {}
        for attrib_name, linear in self.linears.items():
            logits[attrib_name] = linear(x)
        
        return logits

class AttribLinear(nn.Module):
