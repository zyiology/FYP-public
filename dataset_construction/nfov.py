...
class NFOV():
    ...
    def toNFOV(self, frame, center_point):
        ...
        nfov = self._bilinear_interpolation(spericalCoord)

        return nfov

        # if output_file:
        #     plt.imsave(output_file, nfov)
        # else:
        #     plt.imshow(nfov)
        #     plt.show()
...
if __name__ == '__main__':
    ...
    img = im.imread('images/archive/image_6_pano.jpg')
    nfov = NFOV(height=500, width=1000, fov=(0.6, 0.6))
    center_point = np.array([0.8196, 0.47779])  # camera center point (valid range [0,1])
